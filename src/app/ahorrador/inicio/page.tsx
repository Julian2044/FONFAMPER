        ...
        <Card>
          ...
          <div className="mt-5 space-y-4">
            {[
              ...
              <div
                key={label}
                className={index === 4 ? "flex justify-between rounded-2xl bg-blue-50 p-4 font-extrabold text-[#062b5f]" : "flex justify-between border-b border-slate-100 pb-4 text-sm"}
              >
                <span className="text-slate-500">{label}</span>
                <span className="font-bold text-slate-950">{formatCOP(Number(value))}</span>
              </div>
            ))}
          </div>
        </Card>
